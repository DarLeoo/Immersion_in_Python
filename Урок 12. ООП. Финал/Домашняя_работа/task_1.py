...
class Name:
    ...
    def __set__(self, instance, value):
        self.check_valeu(value)
        setattr(instance, self._param_name, value)

    def check_valeu(self, value):
        if not value.istitle():
            raise AttributeError('Имя должно начинаться с заглавной буквы')
        if not isinstance(value, str):
            raise AttributeError('Имя должно являться строкой')
        if not value.isalpha():
            raise AttributeError('Имя должно включать только буквы')
...
class Student:
    ...
    def __str__(self):
        ...
        return f'Студент: {self._first_name} {self._last_name}\n' \
               f'Средняя оценка по предметам: {self.avg_grades()}\n' \
    ...
    def validate_nums(self, value, min_num, max_num):
        if not min_num <= value <= max_num:
            raise ValueError('Такой оценки нет')
    ...
    def grade(self, item, value):
        ...
        self.validate_nums(value, min_grade, max_grade)
